...
if __name__ == "__main__":
    ...
    sim_dir = './contents/'
    for i in range(10):
        path = str(i) + '_' + str(i+1)
        if not os.path.exists(path):
            os.mkdir(path)

    gen_imgs = os.listdir(gen_dir)
    for gen_idx in gen_imgs:
        idx = gen_idx.split('-')[0]
        img = cv2.imread(gen_dir + gen_idx)
        if  img is not None:
            gen_img = load_images(gen_dir + gen_idx)
            sim_img = load_images(sim_dir+ idx + '.jpg')
            ...
            cat =  int(true_vs_false*10//1)
            #print(cat)
            shutil.copy(gen_dir + gen_idx,str(cat) + '_' + str(cat+1) +'/'+ gen_idx)
